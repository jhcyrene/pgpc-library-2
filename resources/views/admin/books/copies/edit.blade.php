<x-layout.admin>
    <div class="flex-1 flex flex-col min-h-0 h-full p-6 bg-gray-50/50">
        <div class="flex items-center justify-between mb-6 shrink-0">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Physical Copy</h1>
                <p class="text-sm text-gray-500 mt-1 font-medium">{{ $book->bookData->book_title }}</p>
            </div>
            
            <a href="{{ route('admin.books.copies.index', $book->book_data_id) }}" class="flex items-center gap-2 text-[#1A2B56] bg-white border border-gray-200 hover:bg-gray-50 text-sm font-bold px-4 py-2.5 rounded-lg transition-all shadow-sm">
