<dialog id="admin-book-details-modal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box w-11/12 max-w-5xl bg-gray-50 p-0 overflow-hidden flex flex-col h-[90vh]">
        ...
        <div class="bg-white p-4 md:p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 z-20 shrink-0">
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="dialog">
                    <button class="btn btn-sm btn-circle btn-ghost" onclick="document.getElementById('admin-book-details-modal').remove()">✕</button>
                </form>
            </div>
        </div>

        <div class="p-4 md:p-6 overflow-y-auto flex-1 color-white">
            <div class="space-y-4">
                
                <!-- Main Book Details Card -->
                <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-6">
                    <!-- Cover Image -->
                    <div class="w-40 shrink-0 mx-auto md:mx-0">
                    ...
                    @endphp
                    @if($firstCopy)
                        <div class="w-48 shrink-0 flex flex-col items-center justify-center p-3 bg-gray-50 border border-gray-100 rounded-lg mx-auto md:mx-0">
...
<!-- Inner Barcode & QR Code Modal -->
<div id="innerBarcodeModal" class="fixed inset-0 z-[60] hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="closeModalBarcodeView()"></div>
                    ...
                    <div class="text-center">
                        ...
                        <div class="flex flex-col items-center gap-6">
                            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 w-full flex flex-col items-center">
                                ...
                                <div id="inner-modal-qr-container" class="bg-white p-3 rounded-lg shadow-sm border border-gray-100 inline-block"></div>
                            </div>
                            
                            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 w-full flex flex-col items-center">
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Barcode</span>
                                <img id="inner-modal-barcode-img" src="" alt="Barcode" class="h-16 object-contain mix-blend-multiply bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-100 w-full">
                                <p id="inner-modal-barcode-text" class="mt-2 text-lg font-mono font-bold tracking-widest text-gray-800"></p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    <button type="button" onclick="closeModalBarcodeView()" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Close</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function showModalBarcodeView(id, barcodeText) {
        const qrHtml = document.getElementById('modal-qr-data-' + id).innerHTML;
        const barcodeBase64 = document.getElementById('modal-barcode-data-' + id).innerHTML;
        
        document.getElementById('inner-modal-qr-container').innerHTML = qrHtml;
        document.getElementById('inner-modal-barcode-img').src = barcodeBase64;
        document.getElementById('inner-modal-barcode-text').innerText = barcodeText;
        
        document.getElementById('innerBarcodeModal').classList.remove('hidden');
    }

    function closeModalBarcodeView() {
        document.getElementById('innerBarcodeModal').classList.add('hidden');
    }
</script>
