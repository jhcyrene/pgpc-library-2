...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (data.success && data.stats) {
                ...
                if (borrowersTbody && data.current_borrowers) {
                    if (data.current_borrowers.length === 0) {
                        borrowersTbody.innerHTML = '<tr><td colspan="4" class="px-5 py-8 text-center text-slate-500 font-medium">No current borrowers found.</td></tr>';
                    } else {
                        borrowersTbody.innerHTML = data.current_borrowers.map(b => `
                            <tr class="hover:bg-slate-50 transition-colors group">
                                <td class="px-5 py-4 font-bold text-slate-900 group-hover:text-blue-600 transition-colors">${b.member_name}</td>
                                <td class="px-5 py-4 font-medium">${b.book_title}</td>
                                <td class="px-5 py-4 text-slate-500 text-xs">${b.borrow_date}</td>
        ...
        .catch(err => {
            ...
            const borrowersTbody = document.getElementById('dashboard-borrowers-tbody');
            if (borrowersTbody) borrowersTbody.innerHTML = '<tr><td colspan="4" class="px-5 py-8 text-center text-red-500 font-medium">Failed to load data.</td></tr>';
