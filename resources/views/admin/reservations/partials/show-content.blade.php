<div class="flex flex-col sm:flex-row justify-between sm:items-start gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-[#102b70]">Reservation Details</h1>
        <p class="text-sm text-gray-500 mt-1">Reservation #{{ $reservation->book_request_id }}</p>
    </div>
    <div class="flex items-center gap-2">
        <!-- Actions are handled in the status card, but we could put them here if we wanted. For now, leave empty or put global actions -->
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
            <h3 class="text-lg font-bold text-[#102b70] border-b border-gray-100 pb-4 mb-4">Book Information</h3>
            <div class="flex flex-col md:flex-row gap-6">
                @if($reservation->bookData->bookDetail && $reservation->bookData->bookDetail->image_url)
                    <div class="w-24 h-36 shrink-0 bg-gray-100 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                ...
                @else
                    <div class="w-24 h-36 shrink-0 bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center">
        ...
        <!-- Member Info -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
            <h3 class="text-lg font-bold text-[#102b70] border-b border-gray-100 pb-4 mb-4">Student Information</h3>
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-full bg-blue-100 text-brand-navy flex items-center justify-center text-2xl font-bold">
                ...
                <div>
                    ...
                    <div class="text-sm text-gray-500">Student Number: <span class="font-medium text-gray-700">{{ $reservation->member->student_number }}</span></div>
                    <div class="text-sm text-gray-500">Email: <span class="font-medium text-gray-700">{{ $reservation->member->email ?? 'N/A' }}</span></div>
                </div>
            </div>
        </div>
