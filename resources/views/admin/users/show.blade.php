<x-layout.admin>
    <x-admin.page-header 
        ...
        description="{{ ucfirst($type) }} Profile"
        :breadcrumbs="[
    ...
    >
        <x-slot:actions>
            ...
            <form action="{{ $type === 'member' ? route('admin.members.destroy', $user->member_id) : route('admin.librarians.destroy', $user->librarian_id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete or deactivate this {{ $type }}?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-error btn-outline btn-sm bg-white">Delete</button>
            </form>
        </x-slot:actions>
    </x-admin.page-header>
    ...
    @endif
    @if(session('error'))
        <div class="alert alert-error mb-6 rounded-lg shadow-sm">
