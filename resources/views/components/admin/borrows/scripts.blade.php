...
(function() {
    ...
    window.openDetails = function(groupId) {
        ...
        document.getElementById('modal-subtitle').textContent = `Loaned on ${item.issue_date}`;

        let booksHtml = item.books.map(book => {
            let statusClass = book.status === 'Returned' ? 'text-green-600 bg-green-50' : 'text-blue-600 bg-blue-50';
            let statusIcon = book.status === 'Returned' 
                ? '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>'
                : '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>';

            const coverHtml = book.cover 
                ? `<img src="${book.cover}" class="w-full h-full object-cover rounded-md">`
                : `<div class="w-full h-full bg-gray-100 flex items-center justify-center text-gray-300 rounded-md"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg></div>`;

            return `
            <div class="flex items-start gap-4 p-4 border border-gray-100 rounded-xl bg-white shadow-sm hover:border-gray-200 transition-colors">
                <div class="w-12 h-16 shrink-0 shadow-sm">
                    ${coverHtml}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="font-bold text-gray-900 leading-tight mb-1 truncate">${book.title}</p>
                    <p class="text-xs text-gray-500 truncate">${book.authors}</p>
                    <p class="text-[11px] text-gray-400 mt-1 font-mono">${book.barcode}</p>
                </div>
                <div class="shrink-0 flex flex-col items-end">
                    <div class="flex items-center gap-1.5 px-2 py-1 rounded-md ${statusClass} text-[11px] font-bold">
                        ${statusIcon}
                        ${book.status}
                    </div>
                    ${book.return_date ? `<p class="text-[10px] text-gray-500 mt-2">Returned: ${book.return_date}</p>` : ''}
                </div>
            </div>
            `;
        }).join('');
        ...
        const content = `
            ...
            <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100 mb-6">
                <div class="w-14 h-14 rounded-full bg-gray-200 border-2 border-white shadow-sm overflow-hidden flex items-center justify-center font-bold text-xl text-gray-500">
                    ${item.member.avatar ? `<img src="${item.member.avatar}" class="w-full h-full object-cover">` : item.member.initial}
                </div>
                <div>
                    <h4 class="font-bold text-gray-900 text-lg">${item.member.name}</h4>
                    <p class="text-sm text-gray-500">Student ID: ${item.member.student_id}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div class="bg-white p-3 rounded-lg border border-gray-100 shadow-sm text-center">
                    ...
                    <p class="font-bold text-gray-900 mt-1">${item.status}</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-gray-100 shadow-sm text-center">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Due Date</p>
                    <p class="font-bold text-gray-900 mt-1 ${item.due_state === 'overdue' ? 'text-red-600' : ''}">${item.due_date}</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-gray-100 shadow-sm text-center">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Books</p>
                    <p class="font-bold text-gray-900 mt-1">${item.book_count}</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-gray-100 shadow-sm text-center">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Fine</p>
                    <p class="font-bold ${item.total_fine > 0 ? 'text-red-600' : 'text-gray-900'} mt-1">₱${parseFloat(item.total_fine).toFixed(2)}</p>
                </div>
            </div>

            <!-- Books List -->
            <div>
                <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Borrowed Books</h4>
                <div class="space-y-3">
                    ${booksHtml}
                </div>
            </div>
        ...
        // Dynamically set modal actions
        let actionsHtml = `
            <form method="dialog" class="w-full sm:w-auto">
                <button class="btn btn-ghost font-medium w-full sm:w-auto">Close</button>
            </form>
        `;
        if (item.total_fine > 0) {
            actionsHtml += `
            <button class="btn bg-green-600 hover:bg-green-700 text-white font-bold shadow-md shadow-green-500/20 border-none w-full sm:w-auto" onclick="window.payFines('${item.group_id}')">
                Mark as Paid
            </button>
            `;
        }
        actionsHtml += `
            <button class="btn bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-md shadow-blue-500/20 border-none w-full sm:w-auto" onclick="window.location.href='{{ route('admin.circulation.index') }}'">
                Go to Circulation Desk
            </button>
        `;
        document.getElementById('modal-actions').innerHTML = actionsHtml;
        
        document.getElementById('loan-details-modal').showModal();
    };
    ...
    window.payFines = async function(groupId, event = null) {
        ...
        showLoading(true);
        try {
            const response = await fetch('{{ route('admin.api.borrows.pay') }}', {
                method: 'POST',
                headers: {
    ...
    async function loadStats() {
        try {
            const response = await fetch('{{ route('admin.api.borrows.stats') }}', {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
                }
    ...
    async function loadData() {
        ...
        const url = new URL('{{ route('admin.api.borrows.list') }}');
