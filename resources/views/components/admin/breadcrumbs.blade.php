    ...
    <nav class="text-sm font-medium text-gray-500 mb-2" aria-label="Breadcrumb">
        <ol class="list-none p-0 inline-flex flex-wrap items-center gap-2">
            @foreach($breadcrumbs as $breadcrumb)
                @if(!$loop->last)
                    <li class="flex items-center">
                        @if(isset($breadcrumb['url']))
                            <a href="{{ $breadcrumb['url'] }}" class="hover:text-[#102b70] transition-colors">{{ $breadcrumb['label'] }}</a>
                        @else
                ...
                @else
                    <li class="flex items-center text-[#102b70] font-semibold" aria-current="page">
                        {{ $breadcrumb['label'] }}
                    </li>
                @endif
            @endforeach
