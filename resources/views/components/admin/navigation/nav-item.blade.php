...
@php
    $classes = 'flex items-center justify-between px-3 py-2.5 lg:py-1.5 rounded-lg transition-all duration-200 group ' .
        ($disabled
...
@if($disabled)
    <div class="{{ $classes }}" aria-disabled="true">
        <div class="flex items-center gap-3">
            ...
            @endif
            <span class="text-base lg:text-sm tracking-wide">{{ $label }}</span>
        </div>
        @if($badge)
