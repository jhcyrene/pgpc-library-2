...
<aside id="admin-sidebar" class="w-70 flex flex-col h-dvh shrink-0 overflow-hidden bg-brand-navy">
    ...
    <div class="h-[60px] flex items-center justify-between px-6 pb-5 pt-5 shrink-0 border-b border-white/5">
        <a href="{{ $staffDashboardRoute }}" class="flex items-center gap-4">
            <div
                class="w-8 h-8 rounded-full border border-white/20 shrink-0 overflow-hidden flex items-center justify-center bg-white">
                <img src="{{ Vite::asset('resources/images/pgpc-logo.jpg') }}" alt="PGPC Logo"
    ...
    <!-- Navigation -->
    <div class="flex-1 overflow-y-auto sidebar-scroller py-4 px-3 space-y-1">

        <x-admin.navigation.nav-item label="Dashboard" :href="$staffDashboardRoute" :active="request()->routeIs('admin.dashboard') || request()->routeIs('librarian.dashboard')">
            <x-slot:icon>
        ...
        <!-- Reservations -->
        <x-admin.navigation.nav-item label="Reservations" :href="route('admin.reservations.index')" :active="request()->routeIs('admin.reservations.*')">
            <x-slot:icon>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <x-admin.navigation.nav-item label="Circulation Desk" :href="route('admin.circulation.index')" :active="request()->routeIs('admin.circulation.*')">
            <x-slot:icon>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <x-admin.navigation.nav-item label="Borrows" :href="route('admin.borrows.index')" :active="request()->routeIs('admin.borrows.*')">
            <x-slot:icon>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <x-admin.navigation.nav-group label="Books" :href="route('admin.books.index')" :active="request()->routeIs('admin.books.*') || request()->routeIs('admin.book-copies.*')" id="nav-group-books">
            <x-slot:icon>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @if ($isAdministrator)
            ...
            <x-admin.navigation.nav-item label="User Management" :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*') ||
                request()->routeIs('admin.members.*') ||
                request()->routeIs('admin.librarians.*')">
                <x-slot:icon>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @else
            <x-admin.navigation.nav-item label="Settings" :href="route('librarian.settings.index')" :active="request()->routeIs('librarian.settings.*')">
                <x-slot:icon>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
