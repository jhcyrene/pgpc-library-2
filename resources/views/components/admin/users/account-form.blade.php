...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
        ...
        <input type="text" name="username" value="{{ old('username', $auth->username ?? '') }}" class="input input-bordered w-full" />
    ...
    @if($isEdit && $auth)
        <div>
            ...
            <select name="account_status" class="select select-bordered w-full">
    ...
    <div class="md:col-span-2 mt-4">
        <h3 class="text-sm font-bold text-gray-700 border-b border-gray-100 pb-2 mb-4">
            {{ $isEdit && $auth ? 'Change Password (Leave blank to keep current)' : 'Password Setup' }}
        </h3>
    </div>

    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Password {!! !$isEdit || !$auth ? '<span class="text-red-500">*</span>' : '' !!}</label>
        <input type="password" name="password" class="input input-bordered w-full" placeholder="{{ $isEdit && $auth ? 'Leave blank to keep unchanged' : 'Minimum 8 characters' }}" />
        @error('password') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
    </div>

    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Confirm Password {!! !$isEdit || !$auth ? '<span class="text-red-500">*</span>' : '' !!}</label>
        <input type="password" name="password_confirmation" class="input input-bordered w-full" placeholder="Re-type password" />
    </div>
</div>
