<div class="relative z-20 w-full max-w-md px-6 py-12 flex flex-col">
    ...
    <div class="bg-white rounded-3xl shadow-elegant p-8 md:p-10 transform transition-all">
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-primary transition-colors">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 text-sm">Enter your student email and we'll send a six-digit verification code.</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('forgot-password.send') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="email" placeholder="student@pgpc.edu.ph" class="w-full pl-11 pr-4 py-3 bg-gray-50 border {{ $errors->has('email') ? 'border-red-400' : 'border-gray-200' }} rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-gray-400" />
                </div>
