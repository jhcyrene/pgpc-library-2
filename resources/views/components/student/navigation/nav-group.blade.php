...
<li>
    <div class="flex items-center rounded-lg transition-all duration-200 group relative {{ $active ? 'bg-white/10' : 'hover:bg-white/5' }}">
        @if($active)
            <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#FFC107]"></span>
        @endif
        ...
        <a href="{{ $href }}"
           class="min-w-0 flex-1 flex items-center gap-3 pl-3 pr-1 py-2.5 text-sm {{ $active ? 'text-white font-bold' : 'text-slate-300 group-hover:text-white' }}">
            <svg class="h-5 w-5 shrink-0 {{ $active ? 'text-[#FFC107]' : 'text-slate-400 group-hover:text-slate-200' }} transition-colors"
        ...
        <button type="button"
                class="p-3 text-slate-400 hover:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#FFC107]/50"
                aria-label="Toggle {{ $label }} links"
                aria-controls="{{ $id }}"
                aria-expanded="{{ $active ? 'true' : 'false' }}"
                onclick="toggleStudentNavGroup('{{ $id }}')">
            <svg id="{{ $id }}-chevron" class="h-4 w-4 transition-transform duration-200 {{ $active ? 'rotate-90 text-[#FFC107]' : '' }}"
                 fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
