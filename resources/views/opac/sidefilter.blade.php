    ...
    <form action="{{ route('opac.index') }}" method="GET"
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-xl bg-blue-50 text-[#102b70] flex items-center justify-center font-bold shrink-0">
                ...
                @foreach ($statusOptions as $value => $label)
                    <label class="flex items-center gap-2.5 text-xs font-semibold text-slate-700 hover:text-slate-900 cursor-pointer">
                        <input type="checkbox" name="status[]" value="{{ $value }}"
                            ...
                            class="h-4 w-4 rounded border-slate-300 text-[#102b70] accent-[#102b70] focus:ring-[#102b70]/20">
                        {{ $label }}
                    </label>
                @endforeach
            </div>
        ...
        <div class="border-t border-slate-100 pt-4">
            ...
            <div class="flex items-center gap-2">
                <input type="number" name="year_from" inputmode="numeric" placeholder="From"
                    ...
                    class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-800 outline-none transition focus:bg-white focus:border-[#102b70]">
        ...
        <div class="border-t border-slate-100 pt-4">
            ...
            <div class="relative">
                <select id="catalog-sort" name="sort"
                    class="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 pr-9 text-xs font-semibold text-slate-800 outline-none transition focus:bg-white focus:border-[#102b70]">
        ...
        <div class="pt-2">
            <button type="submit"
                class="w-full py-2.5 bg-[#102b70] hover:bg-[#0b225e] text-white font-bold text-xs rounded-xl transition-all shadow-2xs">
                Apply Filters
            </button>
        </div>
    </form>
